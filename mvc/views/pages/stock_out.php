...
<div class="col-lg-12 shadow p-2 mb-3 bg-light rounded">
    <h5 class="text-center text-uppercase my-3">danh sách yêu cầu xuất hàng</h5>
    <div class="d-flex flex-row-reverse">
        <a role="button" aria-pressed="true" class="mb-3" data-toggle="modal" data-target="#newStockOutRequest">
    ...
    <table class="table text-center table-hover">
        ...
        if ($isShow) {
            ...
            while ($res = $listRequest->fetch_assoc()) {
                $id = $res["id"];
                ?>
                <tr id="<?= $id ?>" class="table-row-stock-out">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newStockOutRequestLabel">Tạo yêu cầu xuất hàng mới</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="<?= $root . "Home/NewStockOutRequest" ?>" method="post">
